...
export default async function PMDashboard({ params }: { params: Promise<{ workspace: string }> }) {
    ...
    return (
        <div className="flex flex-col h-full w-full p-10 overflow-y-auto animate-in fade-in slide-in-from-bottom-4 duration-300">
            ...
            <DashboardOverview userId={profile.id} workspaceId={workspace.id} workspaceSlug={workspace.slug} userRole="Project Manager" />
        </div>
    )
}
