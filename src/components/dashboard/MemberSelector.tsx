...
export const MemberSelector = forwardRef<SelectorHandle, MemberSelectorProps>(({ 
  ...
  return (
    <div className={`relative ${align === 'right' ? 'w-full flex justify-end' : ''}`} ref={dropdownRef}>
      ...
      {isOpen && (
        <div className={`absolute left-1/2 sm:left-auto -translate-x-[20%] sm:translate-x-0 ${align === 'left' ? 'sm:left-0' : 'sm:right-0'} top-full mt-2 w-52 sm:w-64 bg-white border border-gray-100 rounded-xl shadow-2xl z-[80] overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200`}>
